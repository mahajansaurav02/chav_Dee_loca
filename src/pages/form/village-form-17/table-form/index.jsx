...
function StepForm() {
  ...
  const deleteForm17 = (record) => {
    ...
    setRecord({ ...record, cCode: codeVillage });
    setModalForDelete(true);
  };

  const deleteForm17ById = async () => {
    setModalForDelete(false);
    //console.log('id in state', record);
    //alert('id' + recordId + '  makta khata' + record.makhtaKhataNo);
    sendRequest(
      `${URLS.BaseURL}/form17/discardForm17?id=${recordId}`,
      'DELETE',
      null,
      (res) => {
        if (res.status === 226) {
        ...
        setDisableFlagButton(true),
          setDataSource(
            res.data.form17Data.map((row, index) => ({
              ...
              demandGenerated: row.demandGenerated,
