...
function Wasuli_without_single_demand() {
  ...
  const handleOnChangeDistrict = async (e) => {
    let year = location?.state?.selectedYear;
    // setTableData();
    await axios
      .post(
        year == '2024-2025'
          ? `${URLS.AuthURL}/getCountWithOutSingleDemandGeneration?districtCode=${location?.state?.districtCode}&talukaCode=${location?.state?.talukaCode}`
          : `${URLS.AuthURL}/getCountWithOutSingleDemandGenerationPre?districtCode=${location?.state?.districtCode}&talukaCode=${location?.state?.talukaCode}`,
      )
      .then((res) => {
        console.log(res, 'Wasuli_without_single_demand-------->');
        try {
          setTableData(
            res.data.map((row, i) => ({
              srNo: i + 1,
          ...
          console.log(err, 'Error in block Wasuli_without_single_demand');
        }
      });
  };
...
export default Wasuli_without_single_demand;
