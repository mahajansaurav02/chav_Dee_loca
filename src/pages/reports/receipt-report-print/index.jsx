...
function ReceiptReportPrint() {
  ...
  useEffect(() => {
    if (!location.state) {
      history.push('/reports/receipt-reports');
    }
    if (receipts) {
      receipts.map((row) => {
        setTotalAmount((prevTotalAmount) => prevTotalAmount + row.netAmount);
      });
    }
  }, [receipts]);
...
export default ReceiptReportPrint;
