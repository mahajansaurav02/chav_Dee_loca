...
function AkrushikSurveyNumber() {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'akrushikAssessment',
    },
  ];

  const showSurveyForm = async () => {
    sendRequest(
      `${URLS.BaseURL}/reports/Form2SurveyNoDetails?districtCode=${districtCode}&talukaCode=${talukaCode}&cCode=${codeVillage}&revenueYear=${revenueYear}`,
      'GET',
      null,
      (res) => {
        setSurveyDetails(
          res.data.form2SrNoData.map((r, i) => ({
            id: r.id,
            srNo: i + 1,
            khataNo: r.khataNo,
            khatedarNames: r.khatedarNames,
            area: r.area,
            akrushikAssessment: r.akrushikAssessment,
            pin: r.pin,
            //    unit: r.unit,
          })),
        );
        ...
        <Row>
          <Col xl={22} lg={22} md={22} sm={20} xs={20}>
            <h2 style={{ textAlign: 'center', marginBottom: '20px' }}>
              <FormattedMessage id="formLanguage.title.akrushikSurveyNo" />
            </h2>
          </Col>
        </Row>
        <Row>
...
export default AkrushikSurveyNumber;
